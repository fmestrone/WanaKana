...
const valueObserver = new MutationObserver((mutations) => {
  mutations.forEach(({ target, oldValue }) => {
    console.log('value mutation', {
      ...
      newGetAttrValue: target.getAttribute('value'),
      oldValue,
    });
  });
});
...
const onKeypress = ({ which }) =>
  console.log(`keypress: ${String.fromCharCode(which)}`);
...
const onCompositionUpdate = (event) => {
  ...
  if (!isFirstLetterN && isDoubleConsonant) {
    console.log('Ignoring next input event!', 'tomato');
  }
};
