...
wanakana._onInput = function(event) {
  ...
  if (input.dataset.wanakanaMode === 'katakana') {
    newText = wanakana._romajiToKatakana(normalizedInputString, {
      IMEMode: true
    });
  } else if (input.dataset.wanakanaMode === 'hiragana') {
    newText = wanakana._romajiToHiragana(normalizedInputString, {
      IMEMode: true
    });
    ...
    newText = wanakana._romajiToKana(normalizedInputString, {
      IMEMode: true
    });
  }
...
wanakana._romajiToKatakana = function(roma, options) {
  return wanakana._hiraganaToKatakana(wanakana._romajiToKana(roma, options, true));
};
...
wanakana._romajiToKana = function(roma, options, ignoreCase) {
  ...
  while (cursor < len) {
    ...
    while (chunkSize > 0) {
      ...
      if (__indexOf.call(wanakana.FOUR_CHARACTER_EDGE_CASES, chunkLC) >= 0 && (len - cursor) >= 4) {
        ...
        if (chunkLC.charAt(0) === "n") {
          if (chunkLC.charAt(1) === "'" && chunkSize === 2) {
            kanaChar = "ん";
            break;
          }
    ...
    if (options != null ? options.useObsoleteKana : void 0) {
      if (chunkLC === "wi") {
        kanaChar = "ゐ";
      }
      if (chunkLC === "we") {
...
wanakana.hasHiragana = function(input) {
  var chars;
  chars = input.split("");
  return chars.some(wanakana._isCharHiragana);
};

wanakana.hasKatakana = function(input) {
  var chars;
  chars = input.split("");
  return chars.some(wanakana._isCharKatakana);
};

wanakana.hasKana = function(input) {
  var chars;
  chars = input.split("");
...
wanakana.hasRomaji = function(input) {
  var chars;
  chars = input.split("");
  return chars.some(function(char) {
    return (!wanakana.isHiragana(char)) && (!wanakana.isKatakana(char));
