...
wanakana._onInput = function(event) {
  ...
  if (input.dataset.wanakanaMode === 'katakana') {
    newText = wanakana._hiraganaToKatakana(wanakana.toKana(normalizedInputString, {
      IMEMode: true,
      ignoreCase: true
    }));
...
wanakana._romajiToHiragana = function(roma, options) {
  return wanakana._romajiToKana(roma, wanakana._extend({
    ignoreCase: true
  }, options));
};

wanakana._romajiToKana = function(roma, options) {
