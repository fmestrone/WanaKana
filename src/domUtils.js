...
export function bind(input, options = {}) {
  ...
  if (input instanceof Element && ELEMENTS.includes(input.nodeName)) {
    ...
    console.warn(
      'Input provided to Wanakana bind() was not a valid input field.'
    );
  }
}
...
export function unbind(input) {
  ...
  if (trackedListener != null) {
    ...
    console.warn(
      'Input called with Wanakana unbind() had no listener registered.'
...
function findListener(input) {
  return (
    input &&
    LISTENERS.find(({ id }) => id === input.getAttribute('data-wanakana-id'))
  );
}
